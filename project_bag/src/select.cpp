...
void Select::sift_index_data(long long chunk_data_start, long long time_start,
                             long long time_end, IndexData& id, std::ifstream& ifs){
    id.new_size = 0;
...
void Select::make_map(long long time_start, long long time_end, std::string topic, Chunk& c,
    ...
    for(std::map<int, IndexData>::iterator i = c.indexdata.begin(); i != c.indexdata.end(); i++){
        ...
        sift_index_data(c.data_start(), time_start, time_end, id, ifs);
...
void make_map(long long time_start, long long time_end, std::string topic, BagFile& bf, std::ifstream& ifs){
    ...
    for(unsigned int i = 0; i < bf.chunks.size(); i++){
        ...
        Select s;
        s.make_map(time_start, time_end, topic, bf.chunks[i], bf.map_conn, ifs);
